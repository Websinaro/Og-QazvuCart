 ...
 * Next's env substitution — so it's served from this route instead,
 * reading process.env at request time, and exposed at the browser-facing
 * path /firebase-messaging-sw.js via a rewrite in next.config.ts (browsers
 * require the SW file to be served from the root scope it wants to
 * control, and service worker scripts must come from a real URL path, not
 * a route living under /api). Cached for a few minutes since the config
 * rarely changes and this is requested on every page load that registers
 * push.
 */
