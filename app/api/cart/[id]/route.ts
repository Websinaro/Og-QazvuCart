...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const cart = await CartService.updateItem(authUser.userId, Number(params.id), parsed.data.quantity);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const cart = await CartService.removeItem(authUser.userId, Number(params.id));
  return apiSuccess(cart);
}
